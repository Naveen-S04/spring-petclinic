...
environment {
    ...
    stages {
        stage('Build Maven package') {
          ...
          stage('Build docker image') {
            steps {
                ...
                sh 'cd ${destination}; sudo docker build -t ${docker_image}:0.0.1.'
       ...
       stage('Push docker image') {
            steps {
                                sh 'sudo docker tag ${docker_image}:v1 ${dockerhub_repo}/${docker_image}:0.0.1'

                sh 'sudo docker push ${dockerhub_repo}/${docker_image}:0.0.1'
            }
        }
    }
}
